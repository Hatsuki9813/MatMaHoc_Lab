...
int main() {
	...
	string msgstr;
	ifstream infile;
	infile.open("message.bin", ios::in | ios::binary);
    ...
    string ivf;
    ifstream initalvector;
    initalvector.open("ivfile", ios::in | ios::binary);
	if (initalvector.is_open()) {
		getline(initalvector, ivf); // The first line of file should be the IV
		initalvector.close();
	} else {
    ...
    istringstream hex_chars_stream2(ivf);
	unsigned char iv[16];
	int j = 0;
	unsigned int d;
	while (hex_chars_stream2 >> hex >> d)
	{
		iv[j] = d;
		j++;
	}
	...
	for (int i = 0; i < messageLen; i += 16) {
		AESDecrypt(encryptedMessage + i, expandedKey, decryptedMessage + i, iv);
	}
